...
public class WicketExampleRequestCycle extends WebRequestCycle {
	...
	/**
	 * Checks to see if the request was ajax based. If so we send a 404 so that
	 * the org.apache.wicket.ajax.IAjaxCallDecorator failure script is executed.
	 * 
	 * @param e
	 */
	private void handlePageExpiredException(final PageExpiredException e) {
		...
		if (response instanceof BufferedWebResponse) {
			...
			if (bufferedWebResponse.isAjax()) {
				// If there is a better way to figure out if SourcesPage was the
				// request, we should
				// do that.
